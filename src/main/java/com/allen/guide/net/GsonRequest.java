...
public class GsonRequest<T> extends Request<T> {
    ...
    protected Response<T> parseNetworkResponse(NetworkResponse response) {
        try {
            String mJsonString = new String(response.data, HttpHeaderParser.parseCharset(response.headers));
            Log.v(TAG, "parseNetworkResponse" + mJsonString);
            if (mTypeToken == null)

                return Response.success(mGson.fromJson(mJsonString, mClass),
                        HttpHeaderParser.parseCacheHeaders(response));
            else
                return (Response<T>) Response.success(mGson.fromJson(mJsonString, mTypeToken.getType()),
                        HttpHeaderParser.parseCacheHeaders(response));
